...
class BayesClassifier:
    ...
    def train(self) -> None:
        ...
        for i, file in enumerate(files, 1): # type: ignore
            #print(f"Training on file {i} of {len(files)}")
            text = self.load_file(os.path.join(self.training_data_directory, file))
            if(file.startswith(self.neg_file_prefix)): #how does this boolean work?
                tokens = self.tokenize(text)
                self.update_dict(tokens, self.neg_freqs)
            elif(file.startswith(self.pos_file_prefix)): #how does this boolean work?
                tokens = self.tokenize(text)
    ...
    def classify(self, text: str) -> str:
        ...
        # individual feature
        self.load_dict(self.pos_filename)
        self.load_dict(self.neg_filename)
        pos_num_keys = sum(self.pos_freqs.values())
        neg_num_keys = sum(self.neg_freqs.values())
        ...
        # of 0)
        for word in words:
            if (not(word in self.pos_freqs)):
                numerator_pos = 1
            else:
                numerator_pos = (self.pos_freqs[word]+1)
            pos_prob += math.log(numerator_pos/pos_num_keys)
            
            if (not(word in self.neg_freqs)):
                numerator_neg = 1
            else:
                numerator_neg = (self.neg_freqs[word]+1)
            neg_prob += math.log(numerator_neg/neg_num_keys)
